...
export async function generateAPAgingReport(
  ...
  // For MVP, we'll simulate AP aging using expense transactions
  ...
  transactions.forEach((t) => {
    ...
    vendorBalances[vendor] = (vendorBalances[vendor] || 0) + amount;
  ...
  const totalAP = totals.current + totals.days31to60 + totals.days61to90 + totals.over90;

  // Vendor summary
  const vendorSummary = Object.entries(vendorBalances)
    .map(([name, balance]) => ({ vendor: name, balance }))
